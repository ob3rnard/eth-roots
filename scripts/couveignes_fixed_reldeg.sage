...
def cf_couveignes_raw(B, e, L, K, m, method='pari'):
    ...
    Input:
    - 'B': number field elt; in L of the form A^e
    ...
    # NA = cf_relative_norms([B],L,K,[],[],[],0)[0];
    ...
    while prod_crt_p < bound:
        ...
        crt_Jp += [Jp];
    __t = cputime(my_t);
    print("time taken for computation of primes is: {:.2f} [#p:{},{:.2f}/p]".format(__t, len(crt_p), __t/len(crt_p)));
    
    my_t = cputime();
    for i in range(len(crt_p)):
        # print("prime is: ", crt_p[i]);
        crt_Ap += [cf_couveignes_mod_p([B], [ZZ(1)], ZZ(e), NA, L, K, m, crt_p[i],
                                           [crt_Ip[i], crt_Jp[i]])];
...
str_file = '../data/couveignes_fixed_reldeg_' + str(p) + '_' + method

for q in range(3, 300):
    e = q
    
    if (q%p ==0) or (gcd((p-1), e) != 1):
        continue

    if (method=='padic') and (not is_prime_power(q)):
        continue
    ...
    print("Average timings are: ", Times_couv)
